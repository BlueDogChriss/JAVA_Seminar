...
//        Cautare dupa cod contract
        ...
        depozitCautat.setCodContract(1012);
...
//        Creare comparator dupa cod contract
        Comparator<Depozit> comparator = new Comparator<Depozit>() {
            @Override
            public int compare(Depozit depozit1, Depozit depozit2) {
                if (depozit1.getCodContract()==depozit2.getCodContract()){
                    return 0;
                } else {
                    if (depozit1.getCodContract()<depozit2.getCodContract()){
                        return -1;
        ...
        Collections.sort(listaDepozite,comparator);
        k = Collections.binarySearch(listaDepozite,depozitCautat,comparator);
        if(k>=0){
            System.out.println("Depozitul cu codul "+depozitCautat.getCodContract()+":");
            System.out.println(listaDepozite.get(k));
        } else {
            System.out.println("Nu am gasit depozitul cu codul "+depozitCautat.getCodContract());
